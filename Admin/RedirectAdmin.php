...
use Ok99\PrivateZoneCore\PageBundle\Entity\Redirect;
...
class RedirectAdmin extends BaseAdmin
{
    ...
    public function validateSourceUrl(Redirect $redirect, ExecutionContextInterface $context)
    {
        if (!$this->id($this->getSubject()) && $redirect->getSourceUrl()) {
            $foundRedirect = $this->entityManager->getRepository('Ok99PrivateZonePageBundle:Redirect')->findOneBy([
            ...
            if (!$foundRedirect) {
                ...
                if (!$foundRedirect) {
                    $foundRedirect = $this->entityManager->createQuery('
                        SELECT r
                        FROM Ok99PrivateZonePageBundle:Redirect r 
                        WHERE
